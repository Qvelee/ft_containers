...
{
	return _tree.Find(make_pair(key_, mapped_type()));
}
...
{
	if (_tree.Find(make_pair(key_, mapped_type())) == end())
		return 0;
	return 1;
}
...
{
	return _tree.lower_bound(make_pair(key_, mapped_type()));
}
...
{
	return _tree.upper_bound(make_pair(key_, mapped_type()));
}
